...
/*
 * Created by Thkim on 2020/09/26
 *
 * SupportOptional 클래스는 하나의 값을 포함할 수 있습니다.
 * Empty 와 Some 클래스만 SupportOptional 클래스를 상속할 수 있도록
 * sealed class 로 선언합니다.
 */
sealed class SupportOptional<out T : Any>(private val _value: T?) {

    // 클래스가 널이 아닌 값을 가지고 있는지 여부를 반환합니다.
    val isEmpty: Boolean
        get() = null == _value

    // 값을 반환하기 전에 checkNotNull() 함수를 사용하여 널 여부를 확인합니다.
    // 널 값을 반환하려 하는 경우 IllegalStateException 이 발생합니다.
    val value: T
